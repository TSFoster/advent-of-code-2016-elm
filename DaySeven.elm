module DaySeven exposing (answers)
...
supportsTLS ip =
    let
        noHypernetContainsABBA =
            List.filter isHypernet ip
                |> List.all (not << containsABBA)

        supernetContainsABBA =
...
containsABBA : Sequence -> Bool
containsABBA =
    let
        getPossibleCombos : List Char -> List ( Char, Char, Char, Char )
        getPossibleCombos chars =
            case chars of
                a :: b :: c :: d :: rest ->
                    ( a, b, c, d ) :: getPossibleCombos (b :: c :: d :: rest)
...
toIPv7 : String -> Maybe IPv7
toIPv7 string =
    let
        makeSequences : List String -> Maybe (List Sequence)
        makeSequences strings =
            if List.length strings > 2 then
                Nothing
            else
                let
                    supernet =
                        List.head strings |> Maybe.map SupernetSequence

                    hypernet =
                        List.drop 1 strings |> List.map HypernetSequence
                in
                    Maybe.map (\reg -> reg :: hypernet) supernet

        listOfMaybes =
            string
                |> String.split "]"
                |> List.map (String.split "[")
                |> List.map makeSequences
    in
        if List.member Nothing listOfMaybes then
            Nothing
        else
