module DayNine exposing (answers)
...
processInstructions : Int -> List Instruction -> Int
processInstructions length list =
    case list of
        [] ->
            length

        { instructionLength, indexEnd, repeatLength, repeatCount } :: rest ->
            let
                isToBeRepeated instruction =
                    instruction.indexEnd <= (indexEnd + repeatLength)

                newList =
                    dropWhile isToBeRepeated rest
...
getInstructions =
    find All (regex "\\((\\d+)x(\\d+)\\)")
        >> List.filterMap matchToInstruction


matchToInstruction : Regex.Match -> Maybe Instruction
matchToInstruction { match, submatches, index } =
    let
        getInt =
            List.head
        ...
        count =
            getInt (List.drop 1 submatches)

        matchLength =
            String.length match

        instruction =
            Instruction matchLength (index + matchLength)
    in
        Maybe.map2 instruction length count
