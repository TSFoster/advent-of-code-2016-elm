...
roomIsValid { name, checksum } =
    name
        |> String.toList
        |> charCounts
        |> List.sortWith compareCharCounts
        |> List.take 5
        |> List.map Tuple.first
        |> (==) (String.toList checksum)


compareCharCounts : ( Char, Int ) -> ( Char, Int ) -> Basics.Order
compareCharCounts ( char1, count1 ) ( char2, count2 ) =
...
charCounts : List Char -> List ( Char, Int )
charCounts =
    List.foldl
        (\char counts ->
            Dict.update char (\count -> Just ((Maybe.withDefault 0 count) + 1)) counts
        )
        Dict.empty
        >> Dict.toList


roomFromString : String -> Maybe Room
roomFromString string =
    let
        name =
            string |> String.dropRight 6 |> String.filter Char.isLower

        checksum =
            String.slice -6 -1 string

        id =
            string |> String.filter Char.isDigit |> String.toInt |> Result.toMaybe
    in
        Maybe.map (Room name checksum) id
