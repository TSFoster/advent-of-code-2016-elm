module DaySix exposing (answers)
    ...
    let
        ...
        getHighestCount key val ( highest, count ) =
            if fn val count then
                ...
                ( highest, count )
    in
        List.foldl addCounts Dict.empty
            >> Dict.foldl getHighestCount ( Nothing, 0 )
            >> Tuple.first


uncons : List String -> ( List Char, List String )
uncons list =
    let
        unconsed =
            List.filterMap String.uncons list
    in
        ( List.map Tuple.first unconsed, List.map Tuple.second unconsed )
